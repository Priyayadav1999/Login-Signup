...
if user==1:
    ...
    if password1 == password2:
        ...
        if lower>=1 and upper>=1 and special_chr>=1 and digit>=1:
            if os.path.exists("underdetails.json")==True:
                with open("underdetails.json") as file_1:
                    m=json.load(file_1)
                    n=m["user"]
                    for i in n:
                        if i["username"]==username:
                            ...
                            if os.path.exists("underdetails.json")==True:
                                with open("underdetails.json") as file:
                                    k=json.load(file)
                                    l=k["user"]
                                    l.append(dict1)
                                    dict["user"]=l
                                    with open("underdetails.json","w") as file1:
                                        json.dump(dict,file1,indent=4)
                            else:
                                list.append(dict1)
                                k=open("underdetails.json","w")
